...
.. list-table::
   ...
   * - 0
     ...
     - Continue.
   * - 1
     - Release aborted by the zest.releaser integration
     - Treat as an explicit release cancellation.
   * - 2
     - Rating was below ``--min``
     - Treat as a package-quality failure. ``argparse`` also conventionally
       uses 2 for invalid command-line syntax.
   * - 3
     - Rating could not be completed
     - Treat as an operational, data, or configuration error and inspect the
       error message.
